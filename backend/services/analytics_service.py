...
async def ai_script_analyzer(notes: list, status_counts: dict) -> str:
    notes_block = "\n".join(f"- {n}" for n in notes[:120]) if notes else "لا توجد ملاحظات بعد."

    prompt = f"""أنت خبير تسويق ومبيعات متخصص في المدارس والتكوين المهني.
لديك البيانات التالية من وكلاء مبيعات مرزوق أكاديمي:

إحصائيات الحالات:
- بريد صوتي / لم يرد (B.V): {status_counts.get('bv', 0)} عميل
- لم يرد نهائياً (N.R): {status_counts.get('nr', 0)} عميل
- غير مهتم (P.I): {status_counts.get('pi', 0)} عميل
- موعد محجوز (RDV): {status_counts.get('rdv', 0)} عميل

ملاحظات الوكلاء على العملاء:
{notes_block}

المطلوب منك أن تكتب:

## 1. أبرز الاعتراضات
استخرج أهم 5 اعتراضات يواجهها الوكلاء مع العملاء، مرتبة من الأكثر تكراراً إلى الأقل.

## 2. سكريبتات الرد على الاعتراضات
لكل اعتراض من الأهم 3 اعتراضات، اكتب رداً احترافياً بالدارجة المغربية/الفرنسية:
**الاعتراض:** [نص الاعتراض]
**الرد:** [رد احترافي يعترف بالمشكلة ثم يقلب الموقف]

## 3. سكريبت إعلان فيديو (60 ثانية)
اكتب سكريبتاً كاملاً لإعلان فيديو يعالج الاعتراضات الرئيسية:
- **الهوك (0-5 ث):** جملة صادمة تجذب الانتباه
- **المشكلة (5-15 ث):** اعتراض العميل بصوت العميل نفسه
- **الحل (15-40 ث):** ما تقدمه مرزوق أكاديمي تحديداً
- **الدليل الاجتماعي (40-52 ث):** نتيجة ملموسة
- **CTA (52-60 ث):** دعوة واضحة للتصرف

## 4. قوالب واتساب للمتابعة
رسالتان قصيرتان (B.V و N.R) لإعادة التواصل بأسلوب لا يبدو كمبيعات.

الرد منظم بالعناوين أعلاه، بالدارجة المغربية أو الفرنسية حسب السياق، مباشر وقابل للاستخدام فوراً."""
